class LDTranspiler:
    ...
    def visit(self, node):
        ...
        # ---------- Literais ----------
            ...
            return node[1]      # HIGH / LOW
        elif ntype == 'CONST_PIN_MODE':
            return node[1]      # INPUT / OUTPUT
        ...
        # ---------- Funções customizadas ----------
            ...
            ret_type = node[1]
            name = node[2]
            params = ", ".join([f"{ptype} {pname}" for ptype, pname in node[3]])
            inner = "".join(["    " + self.visit(s) for s in node[4]])
